...
def main():
	...
	parser.add_argument("-q", "--query",dest = "query", help = "search query",nargs='+')
	...
	if arguments.file:
		...
		file_name = raw_input("Enter the file into which u want to log: ")
	...
	if arguments.query:
		for i in arguments.query:
			...
			get_url(file, query)
	while(1):
		query = raw_input("Enter the search Query or enter \"exit\" to quit :")
		if not len(query):
			continue
		check_exit(query)
		get_url(file, query)
